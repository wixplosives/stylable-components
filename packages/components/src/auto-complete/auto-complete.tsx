...
export function AutoComplete<T, EL extends HTMLElement = HTMLDivElement>(props: AutoCompleteProps<T, EL>) {
  ...
  return (
    <div className={st(classes.root)} onClick={open}>
      <input value={searchText || ''} onChange={onTextChange} onFocus={open} className={classes.input}></input>
      <Popover show={isOpen} matchWidth className={classes.popover}>
        <Area className={classes.area}>
          <ScrollList items={filteredData} {...listProps} searchControl={[searchText, updateSearchText]}></ScrollList>
        </Area>
      </Popover>
    </div>
  );
